...
const AdminBookings = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      console.log("res.data", response.data.bookings);
  ...
  const handleSaveClick = async () => {
    try {
      await axios.patch(`${urls.BOOKINGS}`, editedBooking, { withCredentials: true });
        ...
        <DataGrid
          ...
          columns={[
            ...
            },
            {
              ...
              headerName: "End Time",
              width: 200,
              valueGetter: (params) => formatDate(params.row.endTime),
            },
            {
              field: "totalPrice",
              headerName: "Price",
            ...
            },
            {
              ...
              renderCell: (params) => (
                <>
                  <Button onClick={() => { setSelectedBooking(params.row); setOpenDialog(true); }}>
                    <VisibilityIcon />
                  </Button>
        ...
        <DialogContent>
          {selectedBooking && (
            <Box>
              ...
              <Paper sx={{ p: 2, my: 1 }}>
                <Typography>User: {selectedBooking.userId?.email || "N/A"}</Typography>
                <Typography>Room: {selectedBooking.roomNumber}</Typography>
                <Typography>Participants: {selectedBooking.participants}</Typography>
                <Typography>Start Time: {formatDate(selectedBooking.startTime)}</Typography>
                <Typography>End Time: {formatDate(selectedBooking.endTime)}</Typography>
